...
#
#     PHX_SERVER=true bin/kove start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :kove, KoveWeb.Endpoint, server: true
end
...
if config_env() == :prod do
  ...
  config :kove, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
...
# Load GROQ API key — from env var, or fall back to ../.env file
groq_key =
      ...
      if File.exists?(env_path) do
        ...
        |> Enum.find_value(fn line ->
          case String.split(line, "=", parts: 2) do
            ["GROQ_API_KEY", val] -> String.trim(val)
            _ -> nil
          end
        end)
      end
